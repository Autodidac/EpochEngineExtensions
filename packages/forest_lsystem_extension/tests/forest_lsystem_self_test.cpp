...
using namespace epoch::extensions::forest_lsystem;
...
int main()
{
    ...
    {
        throw std::runtime_error("forest L-system expansion did not grow");
    }

    const auto segments = build_branch_segments(program, TurtleSettings{1.0F, 22.5F});
    if (segments.size() < 5U)
    {
        throw std::runtime_error("forest L-system segment generation produced too few segments");
    }
